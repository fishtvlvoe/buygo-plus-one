...
<div>
    ...
    <button
        ...
        class="md:hidden fixed top-[10px] left-4 z-[60] p-2 bg-white rounded-lg shadow-md text-slate-600 hover:bg-slate-50 transition">
...
$new_menu_items = array_values(array_filter($new_menu_items, function($item) use ($current_user_id) {
    if (empty($item['permission'])) {
        return true;
    }
    return \BuyGoPlus\Services\SettingsService::helper_can($item['permission'], $current_user_id);
}));
?>
