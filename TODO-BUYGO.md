...
### 1. 修復 Bug：出貨頁面應使用子分頁，不應出現彈跳視窗
**優先級：高**

**問題描述：**
目前出貨頁面的某些操作會彈出 Modal 視窗，應該改為使用子分頁（subpage）的方式呈現，與其他頁面保持一致。

**相關檔案：**
- `/includes/views/pages/shipment-details.php`

**參考實作：**
- 訂單頁面的子分頁實作：`/includes/views/pages/orders.php`（currentView、navigateTo 等邏輯）
