...
|---|-------|------|--------------|------------------|
| 38 | 角色權限頁面 UI 重構 | 顯示 ID 對應、簡化欄位、統一編輯體驗 | UI-01~05 | 5 |
| 39 | FluentCart 自動賦予賣家權限 | 購買指定商品自動獲得賣家身份和預設配額 | FC-01~04 | 4 |
| 40 | 小幫手共享配額驗證 | 防止賣家和小幫手總上架商品數超過限制 | QUOTA-01~03 | 3 |
...
Plans:
- [ ] 38-01-PLAN.md — UI 欄位顯示改造（WP ID + BuyGo ID）
- [ ] 38-02-PLAN.md — 欄位隱藏與移除（賣家類型、發送綁定）
- [ ] 38-03-PLAN.md — 商品限制邏輯統一（預設值、disabled 移除）
