...
## Current Milestone: v3.2 批量上架前端

**Goal:** 實作批量上架的完整前端體驗 — 從商品列表頁的「上架」按鈕進入，經過數量選擇、表單填寫（手動輸入或 CSV 匯入），到批次建立商品。後端 API（Phase 56）已完成，本 Milestone 專注前端。

**Target features:**
- 數量選擇頁：選擇要上架幾個商品（5/10/15/20 或自訂），顯示剩餘配額
- 批量上架表單（手動輸入）：商品卡片表單（名稱、售價、數量、描述），支援新增/刪除列
- 批量上架表單（CSV 匯入）：上傳 CSV 檔案，解析並填入表單
- 桌面/手機版響應式：手機版卡片式、桌面版表格式
- 路由整合：從商品列表頁的「+上架」按鈕進入批量上架流程

**Previous:**
- v3.1 完成 WP 後台 6 Tab 功能、批量上架 API 後端（BATCH-01~04）
...
---
*Last updated: 2026-03-01 after v3.2 milestone start*
