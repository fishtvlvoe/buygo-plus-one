...
**Shipped:** v1.5 (2026-02-20)
**Next:** v2.0 — 後台 UI 統一化（計畫中）

v1.0~v1.5 完成了核心功能：設計系統、Dashboard、搜尋、多賣家權限、LINE 通知、出貨同步、子訂單顯示、商品限制、FluentCart 自動賦予。
...
### Active - v2.0 Milestone (Planning)

- [ ] 後台 UI 統一化（6-Tab 結構、仿 LineHub 設計語言）
- [ ] 角色權限重構（表格簡化、5 大項細粒度權限）
- [ ] 資料管理（訂單/商品/客戶篩選+編輯+刪除）
- [ ] 功能管理（Free/Pro 版本、授權碼機制）
- [ ] 開發者工具合併
- [ ] 預留 API 端點（批量上架、自定義欄位）
