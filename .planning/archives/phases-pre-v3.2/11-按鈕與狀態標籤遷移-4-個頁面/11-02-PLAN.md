...
# 確認桌面版狀態標籤已遷移 (line ~211-219)
grep -n "status-tag-warning\|status-tag-success" admin/partials/shipment-products.php | head -4

# 確認手機版狀態標籤已遷移 (line ~328-337)
sed -n '320,340p' admin/partials/shipment-products.php | grep -c "status-tag"

# 確認 inline Tailwind 已移除
grep -c "bg-yellow-100 text-yellow-800\|bg-green-100 text-green-800" admin/partials/shipment-products.php || echo "No inline Tailwind status styles found"
```
  </verify>
  <done>桌面版 (line ~211-219) 和手機版 (line ~328-337) 狀態標籤已遷移到設計系統 .status-tag classes</done>
</task>
