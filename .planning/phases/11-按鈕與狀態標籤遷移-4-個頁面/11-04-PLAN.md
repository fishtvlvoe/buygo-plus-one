...
3. **狀態指示** (手機版，line ~492-509):
  ...
  <verify>
```bash
# 驗證手機版卡片區域 (card-list) 的按鈕遷移
grep -n "card-list" -A 50 admin/partials/orders.php | grep -c "btn btn-\|status-tag"

# 分別驗證查看詳情按鈕
sed -n '475,485p' admin/partials/orders.php | grep "btn btn-primary"

# 驗證轉備貨按鈕
sed -n '485,495p' admin/partials/orders.php | grep "btn btn-primary"

# 驗證狀態指示標籤
sed -n '490,515p' admin/partials/orders.php | grep -c "status-tag-warning\|status-tag-info\|status-tag-success"
```
  </verify>
  <done>手機版卡片的查看詳情按鈕、轉備貨按鈕、和狀態指示標籤都已遷移完成</done>
</task>
