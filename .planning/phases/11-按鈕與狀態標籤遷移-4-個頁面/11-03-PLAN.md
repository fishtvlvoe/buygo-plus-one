...
# 確認手機版卡片狀態標籤已遷移 (line ~320-327)
sed -n '315,330p' admin/partials/shipment-details.php | grep "status-tag"

# 確認 :class 綁定使用 status-tag classes
grep -n "status-tag-warning\|status-tag-success\|status-tag-neutral" admin/partials/shipment-details.php | head -5

# 確認 inline Tailwind 狀態樣式已移除
grep -c "bg-orange-100 text-orange-600\|bg-green-100 text-green-600\|bg-slate-100 text-slate-600" admin/partials/shipment-details.php || echo "No inline Tailwind status styles found"
```
  </verify>
  <done>手機版卡片狀態標籤 (line ~320-327) 已遷移到設計系統 .status-tag classes，使用 status-tag-warning/success/neutral</done>
</task>
