...
must_haves:
  ...
  key_links:
    ...
    - from: "admin/partials/settings.php"
      to: "includes/api/class-settings-api.php"
      via: "REST API DELETE /settings/templates/{key} (重設為預設值)"
      pattern: "resetTemplate"
...
**強制實作 TMPL-01 需求覆蓋**：確認並新增以下所有模板定義到 templateDefinitions 的 buyer 陣列中（若已存在則確認格式正確）：

1. **product_available（商品上架）**：
```javascript
{
...
4. **shipment_shipped（出貨通知）**（如上方已定義）

同時在 `getVariableDescription` 方法中新增所有新變數的中文說明。
...
3. grep 檢查 TMPL-01 需求覆蓋：`grep -E "product_available|new_order|order_status_changed|shipment_shipped" admin/partials/settings.php | wc -l`（應至少有 4 個 key 定義）
  </verify>
  <done>
前端 templateDefinitions 包含所有 TMPL-01 需求的通知類型（product_available, new_order, order_status_changed, shipment_shipped），每個模板的變數說明已新增到 getVariableDescription
  </done>
</task>
...
<task type="auto">
  <name>Task 3a: 驗證模板儲存 API 和前端編輯器 UI</name>
  <files>
    - includes/api/class-settings-api.php
    - includes/services/class-notification-templates.php
    - admin/partials/settings.php
  ...
  <action>
驗證 Settings API 和前端編輯器 UI 是否已實作，確認符合 TMPL-02 和 TMPL-04 需求。
...
4. **前端模板編輯器 UI（TMPL-02）**
   確認 admin/partials/settings.php 包含以下元素：
   - 模板編輯器（textarea 或類似元素）
   - 變數列表顯示（顯示 product_list, shipping_method, estimated_delivery）
   - 變數點擊複製功能
   - 「儲存」按鈕及其 onclick 事件處理器
   - 確認儲存時呼叫 POST /settings/templates API

   檢查方法：
   ```bash
   # 檢查 textarea 或編輯器元素
   grep -n "textarea\|editor" admin/partials/settings.php | grep -i template

   # 檢查變數列表
   grep -n "variables\|variableList" admin/partials/settings.php

   # 檢查儲存按鈕
   grep -n "saveTemplate\|儲存" admin/partials/settings.php
   ```

**如果發現前端 UI 元素缺失，記錄在 SUMMARY.md 中，因為這可能需要額外的開發工作。**
  </action>
  <verify>
...
5. grep 檢查前端編輯器：`grep -n "textarea\|editor" admin/partials/settings.php | grep -i template | head -5`
6. grep 檢查變數列表：`grep -n "variables" admin/partials/settings.php | head -5`
7. grep 檢查儲存功能：`grep -n "saveTemplate" admin/partials/settings.php | head -3`
  </verify>
  <done>
已驗證：(1) 模板儲存 API 端點，(2) wp_options 儲存機制，(3) 多層快取實作，(4) 前端模板編輯器 UI 元素（textarea、變數列表、儲存按鈕）
  </done>
</task>

<task type="auto">
  <name>Task 3b: 實作「重設為預設值」功能</name>
  <files>
    - admin/partials/settings.php
    - includes/api/class-settings-api.php
  </files>
  <action>
實作「重設為預設值」功能，讓賣家可以清除自訂模板並回復到預設值。

**前端實作**（在 admin/partials/settings.php）：
   - 在每個模板編輯區塊新增「重設為預設值」按鈕
   - 按鈕點擊處理器呼叫 `resetTemplate(templateKey)` 方法
   - 實作 `resetTemplate` 方法：
     ```javascript
     const resetTemplate = async (templateKey) => {
         if (!confirm('確定要重設為預設值嗎？自訂內容將會被清除。')) return;

         try {
             const response = await fetch(
                 `${apiBase}/settings/templates/${templateKey}`,
                 {
                     method: 'DELETE',
...
**後端實作**（在 includes/api/class-settings-api.php）：
   - 確認 DELETE /settings/templates/{template_key} 端點存在
   - 若不存在，需要新增路由和方法：
     ```php
     // 在 register_routes() 中新增路由
     register_rest_route('buygo-plus-one/v1', '/settings/templates/(?P<key>[a-z_]+)', [
         'methods' => 'DELETE',
...
**如果 API 端點已存在，僅需實作前端按鈕和 onclick 處理器即可。**
  </action>
  <verify>
1. grep 檢查 DELETE 端點：`grep -n "DELETE.*templates\|delete_template" includes/api/class-settings-api.php`
2. grep 檢查前端重設按鈕：`grep -n "resetTemplate" admin/partials/settings.php`
3. grep 檢查重設按鈕文字：`grep -n "重設為預設值" admin/partials/settings.php`
4. PHP 語法檢查：`php -l includes/api/class-settings-api.php`
  </verify>
  <done>
已實作：(1) DELETE /settings/templates/{key} API 端點，(2) 前端「重設為預設值」按鈕，(3) resetTemplate() onclick 處理器
  </done>
</task>
...
5. **TMPL-01 需求覆蓋驗證**
   ```bash
   # 確認所有四種通知類型都已定義
   grep -c "product_available\|new_order\|order_status_changed\|shipment_shipped" admin/partials/settings.php
   # 預期結果：至少 4（每種類型至少一個 key 定義）
   ```
...
7. **重設為預設值功能驗證**
   ```bash
   # 確認 DELETE 端點存在
   grep -n "DELETE.*templates\|delete_template" includes/api/class-settings-api.php

   # 確認前端重設按鈕和處理器
   grep -n "resetTemplate\|重設為預設值" admin/partials/settings.php
   ```
</verification>
...
4. 所有 PHP 檔案通過語法檢查
5. templateDefinitions 包含 TMPL-01 需求的所有四種通知類型（product_available, new_order, order_status_changed, shipment_shipped）
6. 已驗證模板儲存 API 使用 buygo_notification_templates wp_option 且實作多層快取
7. 「重設為預設值」按鈕存在且功能完整（前端按鈕 + onclick 處理器 + DELETE API 端點）
</success_criteria>
