...
must_haves:
  truths:
    ...
    - "狀態切換按鈕使用 .status-tag classes 且保持 button 元素（互動式）"
    - "子頁面按鈕使用 .btn classes"
...
<task type="auto">
  <name>Task 2: 遷移狀態切換按鈕</name>
  <files>admin/partials/products.php</files>
  <action>
找到並替換狀態切換按鈕：

**桌面版 Table View 狀態切換按鈕** (這是互動式按鈕，保持為 button 元素):
...
grep -c "status-tag" /Users/fishtv/Development/buygo-plus-one-dev/admin/partials/products.php
```
  </verify>
...
**桌面版表格 icon-only 按鈕**（分配、編輯、刪除）：保持現狀，不遷移。這些按鈕使用 icon-only 設計，有其自身設計語言。
  </action>
  <verify>
```bash
# 驗證手機版卡片按鈕容器存在
...
<output>
After completion, create `.planning/phases/11-按鈕與狀態標籤遷移/11-04-SUMMARY.md`
</output>
