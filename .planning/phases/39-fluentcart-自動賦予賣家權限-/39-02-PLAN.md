...
must_haves:
  truths:
    - "當顧客購買賣家商品並付款完成時，fluent_cart/order_paid hook 被正確監聽並執行賦予流程"
  ...
  artifacts:
    - path: "includes/integrations/class-fluentcart-seller-grant.php"
      ...
      contains: "fluent_cart/order_paid"
...
<objective>
建立 FluentCart 訂單監聽整合類別，當顧客購買指定的賣家商品並付款完成時，自動賦予 buygo_admin 角色和預設配額。
...
2. 在 `register_hooks()` 方法的 FluentCart 整合區塊中新增：
  ...
  <verify>確認 debug.log 中有 `[BuyGo+1][SellerGrant]` 記錄（測試購買後）</verify>
  <done>整合類別已在 Plugin 中正確註冊，Hook 可正常監聽</done>
</task>
