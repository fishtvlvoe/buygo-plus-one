...
class Auto_Updater {
     ...
     * @var string
     */
    private $plugin_slug = 'buygo-plus-one';

    /**
     * 外掛檔案路徑
     * @var string
     */
    private $plugin_file = 'buygo-plus-one/buygo-plus-one.php';
    ...
    public function __construct($version, $api_url = '') {
        ...
        // 允許透過常數覆寫 API URL
        if (defined('BUYGO_UPDATE_API_URL')) {
            $this->api_url = BUYGO_UPDATE_API_URL;
        }
    ...
    private function init_hooks() {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG && isset($_GET['clear_update_cache'])) {
            ...
            add_action('admin_notices', function() {
                echo '<div class="notice notice-success"><p>外掛更新快取已清除</p></div>';
            });
        }
    }
    ...
    public function check_update($transient) {
        ...
        // 檢查快取
        $cached = get_transient('buygo_plus_one_update_check');
        if ($cached !== false) {
        ...
        if (!empty($data['up_to_date']) && $data['up_to_date'] === true) {
            // 快取「已是最新版本」的結果
            set_transient('buygo_plus_one_update_check', [
                'has_update' => false,
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) [
            ...
            'sections' => [
                'description' => $data['description'] ?? 'BuyGo 賣場管理系統 - 提供獨立的後台管理介面',
    ...
    private function get_changelog($data) {
        ...
        $version = $data['version'] ?? $this->version;
        $release_url = $data['release_url'] ?? "https://github.com/fishtvlvoe/buygo-plus-one/releases/tag/v{$version}";
