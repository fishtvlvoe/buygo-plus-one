...
- [x] 1.2 [Tool: cursor] 手動驗證：進入庫存分配頁（全部模式），確認「已下單」數字與下單名單頁的「下單數量」相符（對應 spec: Allocation page ordered count equals buyers page ordered count）
- [x] 1.3 [Tool: cursor] 修復 variant 篩選模式「已下單」數字：目前 variant 模式呼叫 `/variations/{id}/stats` API，其 `ordered` 含已出貨歷史訂單（如 id=353 回傳 5），但下方列表只顯示待分配訂單（加總為 3），上下不一致。
