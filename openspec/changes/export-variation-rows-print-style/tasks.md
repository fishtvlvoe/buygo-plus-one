...
- [ ] 1.3 測試：匯出含多 variation 的出貨單，確認 CSV 輸出兩行（如「產品測試 - (A) 漢頓」qty 3 和「產品測試 - (C) 大耳狗」qty 3）；匯出無 variation 的出貨單，確認 CSV 維持單行輸出。驗證：在 InstaWP 測試站手動匯出，CSV 內容符合 spec 的三個 scenario。[Tool: 手動驗證]
...
- [ ] 2.3 測試：在 InstaWP 測試站開啟出貨單詳情頁，按 Cmd+P 列印預覽，確認樣式符合 spec 的三個 scenario（隱藏非內容元素、保留內容元素、表格排版）。驗證：截圖列印預覽畫面確認。[Tool: 手動驗證]
