...
$customers_component_template .= <<<'HTML'
    ...
    <div v-show="currentView === 'list'" class="p-2 xs:p-4 md:p-6 w-full max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        <smart-search-box
            ...
            :search-fields="['full_name', 'phone', 'email', 'custom_id']"
            display-field="full_name"
            display-sub-field="email"
            display-badge-field="custom_id"
            placeholder="搜尋客戶名稱、電話、Email 或編號..."
            :show-image="false"
            :show-status="false"
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="px-4 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider w-[22%]">客戶</th>
                                <th class="px-2 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider w-[10%] whitespace-nowrap">編號</th>
                        ...
                        <tbody>
                            <tr v-for="customer in customers" :key="customer.id">
                                ...
                                <td class="px-2 py-4 text-left text-sm text-slate-600">{{ customer.custom_id || '—' }}</td>
                ...
                <div v-for="customer in customers" :key="customer.id" class="card">
                    <div class="mb-3">
                        <div class="flex items-start justify-between gap-2 mb-1">
                            <div class="text-base font-bold text-slate-900">{{ customer.full_name || '-' }}</div>
                            <!-- 自訂編號 badge：僅在有值時顯示，手機版省空間不顯示「—」 -->
                            <span v-if="customer.custom_id" class="shrink-0 text-xs text-slate-500 bg-slate-100 rounded-full px-2 py-0.5 leading-tight">{{ customer.custom_id }}</span>
                        </div>
